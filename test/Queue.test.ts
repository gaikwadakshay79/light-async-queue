...
describe("Queue", () => {
  ...
  describe("process", () => {
    ...
    it("should set a processor module config", () => {
      expect(() => {
        queue.process({
          modulePath: "./test/fixtures/mailProcessor.mjs",
          exportName: "sendMailProcessor",
        });
      }).not.toThrow();
    });

    it("should execute inline processor with closure and env context", async () => {
      const inlineQueue = new Queue({
        ...config,
        processorExecution: "inline",
      });

      const capturedSuffix = "captured-context";
      process.env.INLINE_TEST_ENV = "inline-ok";

      inlineQueue.process(async (job) => {
        const payload = job.payload as { base: string };
        return {
          value: `${payload.base}-${capturedSuffix}`,
          envValue: process.env.INLINE_TEST_ENV,
        };
      });

      const jobId = await inlineQueue.add({ base: "job" });
      await inlineQueue.drain();

      const completed = await inlineQueue.getJob(jobId);

      expect(completed?.status).toBe(JobStatus.COMPLETED);
      expect(completed?.result).toEqual({
        value: "job-captured-context",
        envValue: "inline-ok",
      });

      await inlineQueue.shutdown();
      delete process.env.INLINE_TEST_ENV;
    });
  });

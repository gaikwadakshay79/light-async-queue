...
describe("Worker", () => {
  ...
  describe("execute", () => {
    it("should throw error if worker is not initialized", async () => {
      ...
      await expect(worker.execute(jobData)).rejects.toThrow(
  ...
  describe("processor function handling", () => {
    ...
    it("should store a module-based processor configuration", () => {
    ...
    it("should execute in inline mode with closure context", async () => {
      const closureValue = "inline-closure";
      ...
      const result = await inlineWorker.execute(jobData);
      await inlineWorker.terminate();

      expect(result.success).toBe(true);
      expect(result.result).toEqual({ closureValue: "inline-closure" });
    });
  });
});
